...
const TOOLS = [
  ...
  { name: "submit", description: "Create, update or remove your public resume. Keep contact out of markdown. First send name, private email, action and content; enter the emailed code with request_id to confirm. Only confirmed upserts open public PRs. A maintainer reviews before publication. Removal hides your resume immediately after verification. Claims are unverified; email ownership is not identity verification.", inputSchema: {
    type: "object", properties: { action: { type: "string", enum: ["upsert", "remove"], default: "upsert" }, name: { type: "string", pattern: "^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$" }, email: { type: "string", description: "Private email you control; never included in public GitHub content" }, content: { type: "string", description: "Public markdown without email addresses", maxLength: 32768 }, request_id: { type: "string" }, code: { type: "string", pattern: "^[0-9]{8}$" } }, additionalProperties: false,
  } },
] as const;
